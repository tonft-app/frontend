...
export function AuthButton() {
	...
	const handleButtonClick = useCallback(async () => {
		...
		if (isMobile()) {
			openLink(addReturnStrategy(universalLink, 'none'));
		} else {
	...
	return (
		<>
			{wallet ? (
				...
				<BlueButton onClick={() => handleButtonClick()}>{t("mainPage.addWallet")}</BlueButton>
			)}
			<Dialog className={`w-full h-full  absolute bg-black bg-opacity-50 top-0 flex justify-center items-center z-20`} open={!!modalUniversalLink} onClose={() => setModalUniversalLink('')}>
			{/* <Dialog className={`w-full h-full ${!isMobileCustom && "w-[39rem] h-[85rem]"} absolute bg-black bg-opacity-50 top-0 flex justify-center items-center`} open={!!modalUniversalLink} onClose={() => setModalUniversalLink('')}> */}
