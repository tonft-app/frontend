...
import { CardImage, CardSubtitle, CardTitle, CardWrapper, CardWrapperHorizontal, CollectionNameText, PriceWithoutDiscount } from './NFTCardStyles';
...
export const NFTCardHorizontal = ({ url, nftName, nftPrice, onClick, myNFT, collectionName } : NFTCardProps) => {    
    const nftPriceWithoutDiscount = (parseFloat(nftPrice) * 1.1).toFixed(2);
    const collectionNameAdjusted = collectionName?.length! > 13 ?  `${collectionName?.slice(0,13)}...` : collectionName;
    const nftNameAdjusted = nftName.length! > 13 ?  `${nftName.slice(0,13)}...` : nftName;
